...
@request.restful()
def getAllResultList():
    response.view = 'generic.json'
    def GET():
        
        #if not tablename == 'registrered_course': raise HTTP(401, json.dumps({'message': 'Bad Request'}))
        resultlist = db(db.registered_course.id > 0).select()
        
        return dict(resultlist=resultlist)
